...
export default function Home() {
  const postManagerContract = "0x23842c4ddF9812986C52aBfEe3B9745ACd00aB80"; //postManager smart contract address
  ...
  async function getAllPosts() {
    ...
    if (ethereum) {
      ...
      const allPosts = await postManagerContractInstance.getPostsData(
       ...
       setLatestCid(allPosts.postCID);//(3) set latest cid using react set variable
  ...
  async function createPost() {
    try {
      ...
      if (noOfPosts === 0) {
        ...
        setLatestCid(cid); //(4) call web3.storage API function to store data on IPFS as JSON
        ...
        if (ethereum) {
          ...
          let { hash } = await postManagerContractInstance.createPost(
           ...
           await provider.waitForTransaction(hash);// (6) wait for transaction to be mined

           alert(`Transaction sent! Hash: ${hash}`);// (7) display alert message
        }
